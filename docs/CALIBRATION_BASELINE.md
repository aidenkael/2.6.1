...
Formal Bundle 的 `baseline_calibration_version` 必须为：

`runtime-safety-baseline-v1`

否则软件拒绝导入，要求基于当前空基线重新验证和构建。
...
4. 软件导入后先保持 inactive，人工检查版本/规则数量，再手动启用。
5. 启用后做少量已知商品回放，确认 AI → 安全仲裁 → 物流结果符合预期，再扩大使用。
